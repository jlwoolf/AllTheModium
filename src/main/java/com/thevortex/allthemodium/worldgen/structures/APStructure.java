...
public class APStructure extends Structure {

	public static final Codec<APStructure> CODEC = RecordCodecBuilder
			.<APStructure>mapCodec(instance -> instance
					.group(
							APStructure.settingsCodec(instance),
					...
					.apply(instance, APStructure::new))
			.codec();
	 ...
	 * and look for land with air above it and set block pos's Y value to it. Make
	 * sure to set the final boolean in JigsawPlacement.addPieces to false so that
	 * the structure spawns at block pos's y value instead of placing the structure
	 * on the Bedrock roof!
	 *
	...
	public Optional<Structure.GenerationStub> findGenerationPoint(
		...
		if (!APStructure.extraSpawningChecks(context)) {
			return Optional.empty();
		}

		// Set's our spawning block pos's y offset to be 60 blocks up.
		...
		Optional<Structure.GenerationStub> structurePiecesGenerator = JigsawPlacement.addPieces(
				...
				this.projectStartToHeightmap, // Adds the terrain height's y value to the passed in block pos's y value. (This uses WORLD_SURFACE_WG heightmap which stops at top water too)
				// Here, block pos's y value is 60 which means the structure spawn 60 blocks above terrain height.
				// Set this to false for structure to be place only at the passed in block pos's Y value instead.
