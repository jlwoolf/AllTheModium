...
public enum ATMResource implements IResource {
    ALLTHEMODIUM("allthemodium", 16701786, TagRegistry.ALLTHEMODIUM_ORE_ITEM),
    VIBRANIUM("vibranium", 2547336, TagRegistry.VIBRANIUM_ORE_ITEM),
    UNOBTAINIUM("unobtainium", 13718243, TagRegistry.UNOBTAINIUM_ORE_ITEM);
    ...
    public boolean has(ResourceType type) {
        return (
            type != ResourceType.ENRICHED && (!isVanilla || !type.isVanilla())
        );
    }
