 ...
 */
@Plugin(name = "MsgFilter", category = Node.CATEGORY, elementType = Filter.ELEMENT_TYPE)
public class MsgFilter extends AbstractFilter {
    @Override
    public Filter.Result filter(LogEvent event) {
        var message = event.getMessage();
        if(StringUtils.startsWith(message.getFormat(), "Exception loading blockstate definition:")) {
            return Result.DENY;
        }
        else {
